...
namespace Minesweeper
{
    public class Minesweeper
    {
        ...
        {
            string inputPossition = string.Empty;
            ...
            bool areTurnsMixed = false;

            do
            {
                if (isNewGame)
                {
                    Console.WriteLine("Let's go! Play the Minesweeper game. The command 'top' show the ranking," +
                ...
                inputPossition = Console.ReadLine().Trim();
                if (inputPossition.Length >= 3)
                {
                    if (int.TryParse(inputPossition[0].ToString(), out row) && int.TryParse(inputPossition[2].ToString(), out col)
            ...
            while (inputPossition != "exit");
            Console.Read();
        }

        private static void GetRankings(List<Ranking> points)
        {
            Console.WriteLine("\nPoints:");
            if (points.Count > 0)
            {
